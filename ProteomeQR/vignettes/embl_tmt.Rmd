...
```{r read}
(metadatafilename <- file.path(path.package(package = 'ProteomeQR'), 'extdata/embl/tmt/metadata.csv'))
conditions <- read.delim(metadatafilename, sep = ",")
files <- file.path(path.package(package = 'ProteomeQR'), 'extdata', 'embl', 'tmt', unique(conditions$file))
...
for (fn in files)
{
  ...
  x$file <- fn
  keepnames <- as.character(subset(conditions, file == fn)$col.name)
